...
final class MigrationCatalog
{
    ...
    public function all(): array
    {
        ...
        foreach(glob(rtrim($this->directory,'/\\').'/*.sql')?:[] as $path){
            ...
            $risk=in_array($id,$manual,true)?'MANUAL_REVIEW':(preg_match('/\b(DROP\s+(?:TABLE|COLUMN|DATABASE)|DELETE\s+FROM|TRUNCATE)\b/i',$sql)?'DESTRUCTIVE':(preg_match('/\bALTER\s+TABLE\b/i',$sql)?'STRUCTURAL':'SAFE_ADDITIVE'));
