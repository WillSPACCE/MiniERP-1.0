...
final readonly class PersonFiscalData
{
 ...
 public function __construct(array $input)
 {
  ...
  $type=strtoupper($s('person_type',7)?:((strtolower($s('pessoa_fisica'))==='nao')?'PJ':'PF'));
  if(!in_array($type,self::TYPES,true))throw new InvalidArgumentException('Tipo fiscal da pessoa inválido.');
  $doc=strtoupper(preg_replace('/[^A-Za-z0-9]/','',$s('cpf_cnpj',20))??'');$foreign=$s('foreign_id',50);
  if($type==='PF'&&!preg_match('/^\d{11}$/',$doc))throw new InvalidArgumentException('Pessoa física exige CPF com 11 dígitos.');
  if($type==='PJ'&&!preg_match('/^[A-Z0-9]{12}\d{2}$/',$doc))throw new InvalidArgumentException('Pessoa jurídica exige CNPJ válido, inclusive alfanumérico.');
