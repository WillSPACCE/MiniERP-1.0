        ...
        return$this->operations->transaction(function()use($orderId,$idempotencyKey,$userId):array{
            ...
            foreach($order['items']as$item){
                ...
                if($issuer&&$recipient&&!$itemPending){
                    ...
                    $context=new FiscalTaxContext($this->operations->tenantId(),(int)$issuer['id'],(string)$issuer['tax_regime_code'],(string)$issuer['state'],$destinationState,(string)($recipient['person_type']??'PJ'),(string)($recipient['state_registration_indicator']??'9'),(string)($recipient['country_code']??'1058'),(string)$item['ncm'],(string)$item['cest'],(string)$item['merchandise_origin'],(string)$item['tax_benefit_code'],$order['operation_type'],(string)$order['fiscal_model'],(string)$order['purpose'],(bool)$order['final_consumer'],(($recipient['state_registration_indicator']??'9')==='1'),(string)$order['presence_indicator'],new DateTimeImmutable($order['operation_date']),(string)$item['quantity'],(string)$item['unit_price'],$cfopHint?:null,(int)$item['id']);
