...
final readonly class MariaDbTaxRuleRepository implements TaxRuleRepositoryContract
{
    ...
    private function configurationFallback(FiscalTaxContext$c):?FiscalTaxRule
    {
        try{
            ...
            $cst=(string)(($c->finalConsumer?($g['final_consumer_cst_csosn']??null):null)??$ic['cst_csosn']??$g['default_cst_csosn']??'');$icmsRate=(string)(($c->finalConsumer?($ic['final_consumer_rate']??null):null)??$ic['juridica_rate']??$ic['internal_rate']??'');
            $prefix=$c->direction==='ENTRY'?'input':'output';$pisCst=(string)($legacy['pis_'.$prefix.'_cst']??'');$pisRate=(string)($legacy['pis_'.$prefix.'_rate']??'');$cofinsCst=(string)($legacy['cofins_'.$prefix.'_cst']??'');$cofinsRate=(string)($legacy['cofins_'.$prefix.'_rate']??'');
            ...
            if(($legacy['ipi_applicability']??'PENDING')==='APPLICABLE'){$ipiCst=(string)($legacy['ipi_'.$prefix.'_cst']??'');$ipiRate=(string)($legacy['ipi_'.$prefix.'_rate']??'');$cenq=(string)($legacy['ipi_cenq']??'');if($ipiCst===''||$ipiRate===''||$cenq==='')return null;$ipi=['cst'=>$ipiCst,'legal_code'=>$cenq,'base'=>$base,'rate'=>$ipiRate,'amount'=>$amount($ipiRate)];}elseif(($legacy['ipi_applicability']??'PENDING')!=='NOT_APPLICABLE')return null;
