...
final class FiscalIssuedNotesRepository
{
    ...
    {
        $where=['d.tenant_id=?'];$params=[$this->tenantId];$number=$this->numberExpression('r');$artifacts=$this->tableExists('fiscal_artifacts');$statusExpression=$artifacts?'COALESCE(a.status,d.status)':'d.status';
        foreach(['model'=>'r.model','environment'=>'r.environment','series'=>'r.series']as$key=>$column)if((string)($f[$key]??'')!==''){$where[]=$column.'=?';$params[]=$f[$key];}
