...
final class FiscalIssuedNotesRepository
{
    ...
    {
        $where=['d.tenant_id=?'];$params=[$this->tenantId];$number=$this->numberExpression('r');$artifacts=$this->tableExists('fiscal_artifacts');$statusExpression=$artifacts?'COALESCE(a.status,d.status)':'d.status';$resolvedModel="CONVERT(COALESCE(r.model,NULLIF(JSON_UNQUOTE(JSON_EXTRACT(d.totals_json,'$.model')),''),o.fiscal_model) USING utf8mb4) COLLATE utf8mb4_unicode_ci";
