...
a68(!str_contains($bar,'Concluir pedido')&&!str_contains($bar,'Salvar Pedido')&&substr_count($bar,'Prévia DANFE')===1,'duplicate legacy buttons absent');
foreach(['position: sticky','bottom: 8px','min-height: 40px','flex: 0 0 auto','overflow-x: auto']as$needle)a68(str_contains($css,$needle),'OrderActionBarCompactStickyResponsiveTest '.$needle);
...
a68(str_contains($page,'Legado —')&&str_contains($repo,'Compatibilidade: um pedido histórico'),'OrderNatureLegacyCompatibilityTest');
