...
foreach(['validateForm','cpfValid','cnpjValid','Campo obrigatório','if(saving)return','dialog.open||dialog.showModal()'] as $needle)formAssert(str_contains($js,$needle),'resilience '.$needle);
foreach(['grid-template-rows','overflow-y:auto','.md-required','.md-invalid','.md-form>footer .btn'] as $needle)formAssert(str_contains($css,$needle),'visual '.$needle);
