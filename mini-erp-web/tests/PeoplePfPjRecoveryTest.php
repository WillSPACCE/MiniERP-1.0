...
foreach(["type==='PF'","type==='PJ'","button.hidden=false","button.disabled=!isPj","Consulta somente CNPJ","if(form.elements.person_type.value!=='PJ')return",'formatCpf','formatCnpj','finally{saving=false','button.textContent=button.dataset.normalText','if(saving)return','if(!dialog.open)dialog.show()','CPF genérico não é permitido','scrollIntoView'] as $needle)pfPjAssert(str_contains($js,$needle),'controller '.$needle);
